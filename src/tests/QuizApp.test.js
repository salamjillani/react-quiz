...
describe('QuizApp', () => {
  ...
  test('moves to the next question after answering', () => {
    ...
    const parisButton = Array.from(buttons).find(button => button.textContent === 'Paris');
  ...
  test('shows completion screen after all questions', () => {
    ...
    const parisButton = Array.from(firstButtons).find(button => button.textContent === 'Paris');
    fireEvent.click(parisButton);
    fireEvent.click(screen.getByText('Next Question'));

    const secondButtons = screen.getAllByRole('button');
    const waterButton = Array.from(secondButtons).find(button => button.textContent === 'Water');
